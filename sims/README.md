...
- One folder per sim, each picking the right method:
  ...
  - `bicycle` — Explorable (steering trail and bicycle self-balance)
  - `countersteering` — Explorable (wrong-way steering creates the lean that starts a turn)
  - `orbital-transfer` — Canvas 2D (Hohmann transfer geometry)
  - `moon` — Explorable (Moon phases from synced top-down and Earth views)
  - `eclipse-season` — Explorable (phase plus orbital nodes explains eclipse seasons)
  - `gps` — Canvas 2D (trilateration rings)
  - `gears` — Canvas 2D (gear ratios)
  - `orbit` — three.js (3D)
